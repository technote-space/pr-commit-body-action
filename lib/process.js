...
exports.execute = (logger, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const commitTemplate = commits.filter(commit => !pullTitles.includes(commit.original) && !commit.isNotes).map(commit => misc_1.replaceVariables(commit.isChild ? misc_1.getChildCommitTemplate() : misc_1.getCommitTemplate(), [
        { key: 'MESSAGE', value: misc_1.addCloseAnnotation(commit.message, keyword) },
        { key: 'COMMITS', value: commit.commits },
    ])).filter(item => item).join('\n');
    const notesTemplate = commits.filter(commit => commit.isNotes).map(commit => misc_1.replaceVariables(misc_1.getBreakingChangeTemplate(), [
        { key: 'MESSAGE', value: misc_1.addCloseAnnotation(commit.message, keyword) },
        { key: 'COMMITS', value: commit.commits },
    ])).join('\n');
