...
exports.getCommitMessages = (types, exclude, octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    return (yield octokit.paginate(octokit.pulls.listCommits.endpoint.merge(Object.assign(Object.assign({}, context.repo), { 'pull_number': context.payload.number })))).filter((item) => !MERGE_MESSAGE.test(item.commit.message)).map((item) => (Object.assign({ sha: item.sha }, misc_1.parseCommitMessage(item.commit.message, types, exclude))));
});
