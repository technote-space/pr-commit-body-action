...
const getCommitItems = (octokit, context) => __awaiter(void 0, void 0, void 0, function* () {
    const types = misc_1.getCommitTypes();
    const maxNumber = misc_1.getMaxCommitNumber();
    return (yield github_action_version_helper_1.Commit.getCommits(types, misc_1.getExcludeMessages(), misc_1.getBreakingChangeNotes(), octokit, context))
        .map(item => item)
        ...
        if (target) {
            ...
            acc.push(Object.assign(Object.assign({}, item), { commits: [item.sha], children: item.children, notes: item.notes }));
        }
        return acc;
    }, [])
        .reduce((acc, item) => {
        acc.push(Object.assign(Object.assign({}, item), { message: `${item.type}: ${item.message}`, isChild: false, isNotes: false }));
